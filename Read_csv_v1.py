...
while True:
    ...
    if not selected_data.empty:
        ...
        # update the start_time to be the last timestamp plus 15 seconds
        last_timestamp = selected_data[timestamp_col].max()
        start_time = last_timestamp + pd.Timedelta(seconds=15)
        
    else:
        # if there's no data to process, increment the start_time by 15 seconds
        start_time += pd.Timedelta(seconds=15)
